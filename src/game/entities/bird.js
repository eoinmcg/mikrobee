...
export class Bird extends Sprite {
  constructor(g, o) {
    ...
    if (o.i === 'b2') this.flip.x = true;
    this.collidesWith = ['poo'];
  ...
  update(dt) {
    ...
    let g = this.g,
      keys = g.input.keys,
      left = keys[this.controls.left] || this.pad.l,
      right = keys[this.controls.right] || this.pad.r,
      boost = keys[this.controls.boost] || this.pad.f;
  ...
  render() {
    ...
    let q = half / 2;
    if (this.isHolding) {
      g.draw.img(g.imgs[this.isHolding], this.x + q, this.y + half + q, 2);
    }
    if (this.isWearing) {
      g.draw.img(g.imgs[this.isWearing], this.x - 1, this.y - half - 2, 2);
    }
  }
